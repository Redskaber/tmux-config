...
# @usage: import this module in your home-manager config:
#           imports = [ inputs.tmux-config.homeModules.tx-home ];
#           programs.tx.enable = true;
#         Or copy the `home.sessionPath` line into your existing config.
#
...
in
{
  options.programs.tx = {
    ...
    storeDir = lib.mkOption {
      ...
      defaultText = lib.literalExpression ''"''${config.xdg.dataHome}/tx"'';
    ...
    configDir = lib.mkOption {
      ...
      defaultText = lib.literalExpression ''"''${config.xdg.configHome}/tmux"'';
  ...
  config = lib.mkIf cfg.enable {
    ...
    home.sessionVariables = {
      TX_STORE_DIR = cfg.storeDir;
      TX_HOME = cfg.configDir;
    };
  };
}
